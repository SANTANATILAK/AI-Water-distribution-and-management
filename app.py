...
REQUIRED_COLUMNS = ["Temperature", "Population", "Water_Demand"]
...
def load_data():
    ...
    if len(df) < 2:
        raise ValueError("The dataset needs at least two valid rows.")
    return df
...
try:
    ...
    st.error("AquaAI could not start correctly.")
    st.code(str(exc))
    st.stop()
...
if page == "Dashboard":
    ...
    with c1:
        st.markdown(f'<div class="card"><div class="card-title">Records</div><div class="card-value">{len(df):,}</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="card"><div class="card-title">Avg Demand</div><div class="card-value">{df["Water_Demand"].mean():,.1f} L</div></div>', unsafe_allow_html=True)
    with c3:
        st.markdown(f'<div class="card"><div class="card-title">Model R²</div><div class="card-value">{r2:.3f}</div></div>', unsafe_allow_html=True)
    with c4:
        st.markdown(f'<div class="card"><div class="card-title">MAE</div><div class="card-value">{mae:.2f} L</div></div>', unsafe_allow_html=True)

    st.markdown("### Demand Overview")
    chart_df = df[["Temperature", "Water_Demand"]].copy().sort_values("Temperature")
    st.line_chart(chart_df.set_index("Temperature"), height=330)
    ...
    with right:
        if predict:
            input_data = pd.DataFrame({"Temperature": [temp], "Population": [pop]})
            ...
            st.info("Enter the values and click Predict Water Demand.")
    ...
    chart_df = df[["Temperature", "Water_Demand"]].sort_values("Temperature")
    st.bar_chart(chart_df.set_index("Temperature")["Water_Demand"], height=350)

else:
    st.markdown("### About AquaAI")
    st.markdown("""
**AquaAI** is an AI-based water demand prediction dashboard designed to demonstrate how machine learning can support water resource planning.

**How it works**
1. Loads the project dataset from `data/sample.csv`.
2. Cleans and validates the three required columns.
3. Trains a Linear Regression model using temperature and population.
4. Predicts estimated water demand for new conditions.
5. Displays results through an interactive Streamlit dashboard.

**Technology:** Python, Streamlit, Pandas and Scikit-learn.
    """)
